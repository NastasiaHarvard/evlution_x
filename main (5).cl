						...
						__constant uint* layers)
{
	...
	//for (int k = 0; k < N; k++) {
	  // f[(global_id*N*2)+2*k] = w[global_id*2]*w[global_id*2] + w[global_id*2+1]* w[global_id*2+1] - w[global_id*2+1] * (z[2*k] + z[2*k+1]);
       // f[(global_id*N*2+1)+2*k] = w[global_id*2]*w[global_id*2] - w[global_id*2+1]* w[global_id*2+1]  - w[global_id*2] * (z[2*k] + z[2*k+1]);
//		  f[(global_id*N*2)+2*k] = w[0]*w[0] + w[1]* w[1] - w[1] * (z[2*k] + z[2*k+1]);
//           f[(global_id*N*2+1)+2*k] = w[0]*w[0] - w[1]* w[1]  - w[0] * (z[2*k] + z[2*k+1]);
	   f[global_id*2] = w[0]*w[0] + w[1]* w[1] - w[1] * (z[2*global_id]*z[2*global_id]  - z[2*global_id+1]*z[2*global_id+1]);
       f[global_id*2+1] = w[0]*w[0] - w[1]* w[1]  - w[0] * (z[2*global_id] *z[2*global_id]  + z[2*global_id+1]*z[2*global_id+1]);
                    ...
                    __global double* v)
{
    ...
    for (int k = 0; k<2;k++)
    {
        ...
        int index = global_id  * 2 + k;
//       int index = global_id  + k;
